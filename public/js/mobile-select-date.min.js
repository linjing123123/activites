...
(function(root, factory) {
	...
	MobileSelectDate.prototype = {
		...
		default = 0;
			...
			this.settings.max ? this.max = new Date(this.settings.max):
			this.max = new Date();
			this.getData();
			this.bindEvent()
		},
		getData: function() {
			...
			for (var s = this.min.getFullYear(), l = this.max.getFullYear(); s <= l; s++) {
				...
				if (s == this.min.getFullYear()) {
					for (var m = this.min.getMonth() + 1; m <= 12; m++) {
						var o = {};
						o['id'] = o['name'] = ("0" + m).slice(-2);
						o.child = [];
						if (m == this.min.getMonth() + 1) {
							var days = new Date(s, m, 0).getDate();
							for (var d = this.min.getDate(); d <= days; d++) {
								var j = {};
		...
		f: function(data) {
			...
			if (_this.index !== 0 && _this.value[_this.index - 1] == "0" && this.
		default ==0) {
			...
			default ==1) {
				...
				for (var j = _this.
			default, len = item.length; j < len; j++) {
		...
		submit: function() {
			...
			if (this.trigger[0].nodeType == 1) {
				...
				this.trigger.text(this.value.join('-'));
			}
